...
impl ErrorMessage {
    ...
    pub fn try_from_bencoded(input: &BencodedValue) -> Result<Self, String> {
        let dict = match input {
            BencodedValue::Dict(dict) => dict,
            _ => return Err("expected dictionary".to_string()),
        };
        ...
        for (key, value) in dict {
            match key.as_str() {
                ...
                "e" => {
                    ...
                    if list.len() != 2 {
                        return Err("expected list of length 2".to_string());
                    }
                    ...
                    code = match ErrorCode::try_from(code_) {
                        Ok(code) => Some(code),
                        Err(_) => return Err("invalid error code".to_string()),
                    };
                ...
                _ => return Err("unexpected key".to_string()),
            }
        }

        let transaction_id = transaction_id.ok_or("missing transaction ID".to_string())?;
        let code = code.ok_or("missing error code".to_string())?;
        let message = message.ok_or("missing error message".to_string())?;
